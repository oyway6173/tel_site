    ...
    <div class="wrapper">
      ...
      <div class="container">
        <div class="pricing-header p-3 pb-md-3 mx-auto text-left">
          <h1 class="display-5 fw-normal mb-3">Тариф и остатки</h1>
          <h1 class="display-7 fw-normal text-left mb-3">У вас подключен тариф <u>"<?= $deal['deal_name']; ?>"</u></h1>
          <p class="fs-5 text-muted">Абонентская плата по тарифу составляет <?= $deal['price']; ?> ₽ в месяц и включает в себя: <?= $deal['data']; ?> ГБ, <?= $deal['minites']; ?> минут, <?= $deal['price']; ?> СМС и услуги: Безлимит на <?= $deal['socials']; ?>.</p>
        </div>
